...
pub struct PrehashDsaKeyGenerator {
    /// The algorithm to use for key generation
    algorithm: PrehashDsaAlgorithm,
}

impl PrehashDsaKeyGenerator {
    /// Create a new `DsaKeyGenerator` with the specified algorithm
    ///
    /// # Arguments
    ///
    ...
    ///
    /// The new `KeyGenerator`
    pub fn new(algorithm: PrehashDsaAlgorithm) -> PrehashDsaKeyGenerator {
        PrehashDsaKeyGenerator { algorithm }
    }
...
mod test {
    ...
    fn test_key_generator_sign_verify() {
        ...
        let mut key_generator = PrehashDsaKeyGenerator::new(PrehashDsaAlgorithm::MlDsa44);
        let (pk, sk) = key_generator.generate().unwrap();
        ...
        let mut key_generator =
            PrehashDsaKeyGenerator::new(PrehashDsaAlgorithm::MlDsa65EcdsaBrainpoolP256r1);
        let (pk, sk) = key_generator.generate().unwrap();
        assert_eq!(
            pk.get_oid(),
            PrehashDsaAlgorithm::MlDsa65EcdsaBrainpoolP256r1.get_oid()
        );
        assert_eq!(
            sk.get_oid(),
            PrehashDsaAlgorithm::MlDsa65EcdsaBrainpoolP256r1.get_oid()
        ...
        let mut key_generator = PrehashDsaKeyGenerator::new(PrehashDsaAlgorithm::MlDsa44Rsa2048Pss);
        let (pk, sk) = key_generator.generate().unwrap();
        assert_eq!(
            pk.get_oid(),
            PrehashDsaAlgorithm::MlDsa44Rsa2048Pss.get_oid()
        );
        assert_eq!(
            sk.get_oid(),
            PrehashDsaAlgorithm::MlDsa44Rsa2048Pss.get_oid()
        ...
        assert_eq!(
            pk.get_oid(),
            PrehashDsaAlgorithm::MlDsa44Rsa2048Pkcs15.get_oid()
        );
        assert_eq!(
            sk.get_oid(),
            PrehashDsaAlgorithm::MlDsa44Rsa2048Pkcs15.get_oid()
        ...
        let mut key_generator = PrehashDsaKeyGenerator::new(PrehashDsaAlgorithm::MlDsa44Ed25519);
        let (pk, sk) = key_generator.generate().unwrap();
        assert_eq!(pk.get_oid(), PrehashDsaAlgorithm::MlDsa44Ed25519.get_oid());
        assert_eq!(sk.get_oid(), PrehashDsaAlgorithm::MlDsa44Ed25519.get_oid());
        let sig = sk.sign(msg).unwrap();
        assert!(pk.verify(msg, &sig).unwrap());

        // Try a composite algorithm X448 based
        let mut key_generator = PrehashDsaKeyGenerator::new(PrehashDsaAlgorithm::MlDsa87Ed448);
        let (pk, sk) = key_generator.generate().unwrap();
        assert_eq!(pk.get_oid(), PrehashDsaAlgorithm::MlDsa87Ed448.get_oid());
        assert_eq!(sk.get_oid(), PrehashDsaAlgorithm::MlDsa87Ed448.get_oid());
        let sig = sk.sign(msg).unwrap();
        assert!(pk.verify(msg, &sig).unwrap());
    }
}
