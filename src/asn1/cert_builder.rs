...
mod test {
    ...
    use crate::{dsas::DsaAlgorithm, dsas::DsaKeyGenerator, kems::KemAlgorithm, kems::KemKeyGenerator};

    use super::*;

    #[test]
    fn gen_cert_artifacts_with_pure_mldsa() {
        // For pure ML-DSA algorithm only
        let pure_dsa_algs: Vec<DsaAlgorithm> = vec![
            DsaAlgorithm::MlDsa44,
            // DsaAlgorithm::MlDsa65,
            // DsaAlgorithm::MlDsa87,
        ];

        // Inflate all KEM variants
        let kem_algs: Vec<KemAlgorithm> = vec![
        ...
        for dsa_alg in pure_dsa_algs.iter() {
            ...
            let file_name = format!(
                "artifacts/certs/{}-{}_ta.pem",
                dsa_alg_name,
                dsa_alg.get_oid()
            );
            
            // Write the self-signed certificate from TA to the temp directory
            let _ = cert_root.to_pem_file(&file_name);
            
            for kem_alg in kem_algs.iter() {
                // Generate End Entity (EE) Certificate, signed with trusted authority (TA) from previous level
                // Create a leaf (EE) key pair for KEM
                let (pk_kem, _) = KemKeyGenerator::new(*kem_alg).generate().unwrap();
                let builder = CertificateBuilder::new(
                    Profile::Leaf {
                        issuer: cert_root.get_subject(),
                        enable_key_agreement: false,
                    ...
                    subject.clone(),
                    pk_kem,
                    signer,  // Note that the signer is still the TA private key generated by DSA
                )
                .unwrap();
                let cert_kem = builder.build().unwrap();
                ...
                let kem_alg_name = &kem_alg.to_string();

                let file_name = format!(
                    "artifacts/certs/{}_{}-{}_ee.pem",
                    dsa_alg_name,
                    kem_alg_name,
                    kem_alg.get_oid()
                );
                ...
                // Save EE
                let _  = cert_kem.to_pem_file(&file_name);
            }
        }
    }

    #[test]
    fn gen_cert_artifacts_with_composite_mldsa() {
        // For composite ML-DSA algorithm only
        let composite_dsa_algs: Vec<DsaAlgorithm> = vec![
        ...
        for dsa_alg in composite_dsa_algs.iter() {
            // Use DSA to generate key pair for Trust authority
            let (pk_root, sk_root) = DsaKeyGenerator::new(*dsa_alg).generate().unwrap();
