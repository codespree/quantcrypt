...
mod tests {
    ...
    fn test_ml_dsa_44_rsa2048_pss_sha256_self_signed_cert() {
        ...
        let pem_bytes = include_bytes!("../../test/data_old/MlDsa44Rsa2048PssSha256-2.16.840.1.114027.80.8.1.1_ta.pem");
        let pem = std::str::from_utf8(pem_bytes).unwrap().trim();
        let cert = Certificate::from_pem(&pem).unwrap();
        assert!(cert.verify_self_signed().unwrap());
    }
    ...
    fn test_ml_dsa_44_rsa2048_pkcs15_sha256_self_signed_cert() {
        ...
        let pem_bytes = include_bytes!("../../test/data_old/MlDsa44Rsa2048Pkcs15Sha256-2.16.840.1.114027.80.8.1.2_ta.pem");
        let pem = std::str::from_utf8(pem_bytes).unwrap().trim();
        let cert = Certificate::from_pem(&pem).unwrap();
        assert!(cert.verify_self_signed().unwrap());
    }
    ...
    fn test_dsa_kem() {
        ...
        let pem_bytes = include_bytes!("../../test/data_old/MlDsa44-1.3.6.1.4.1.2.267.12.4.4_ta.pem");
        let pem = std::str::from_utf8(pem_bytes).unwrap().trim();
        let cert = Certificate::from_pem(&pem).unwrap();
        assert!(cert.verify_self_signed().unwrap());

        // let child_pem_bytes = include_bytes!("../../test/data/MlDsa44_MlKem512-2.16.840.1.101.3.4.4.1_ee.pem");
        let child_pem_bytes = include_bytes!("../../test/data_old/MlDsa44_MlKem512-1.3.6.1.4.1.22554.5.6.1_ee.pem");
